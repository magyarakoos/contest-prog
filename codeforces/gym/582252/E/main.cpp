...
int32_t main() {
    ...
    for (int i = 1; i < P.size(); i++) {
        if (P[i-1][0][0] == P[i][0][0] && P[i-1][0][1] == P[i][0][1]) {
            // Duplicate point found - one polygon vertex lies on another
            cout << "NO\n";
            return 0;
        }
    }
    ...
    sort(P.begin() + 1, P.end(), [&](node A,node B) {
        ...
        int d1 = (A[0][0] - P[0][0])*(A[0][0] - P[0][0]) + (A[0][1] - P[0][1])*(A[0][1] - P[0][1]);
        int d2 = (B[0][0] - P[0][0])*(B[0][0] - P[0][0]) + (B[0][1] - P[0][1])*(B[0][1] - P[0][1]);
        return d1 < d2;
    });
    ...
    for (int i = 2; i < P.size(); i++) {
        while (hull.size() >= 2 && 
               turn(hull[hull.size()-2][0], hull[hull.size()-1][0], P[i][0]) <= 0) {
            hull.pop_back();
        }
        hull.push_back(P[i]);
    }
    ...
    for (const auto& node : hull) {
        if (node[1][0] == 1) {  // It's a point from polygon B
            cout << "NO\n";
            return 0;
        }
    }
